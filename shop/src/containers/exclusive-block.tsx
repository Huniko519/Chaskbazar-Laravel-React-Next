  ...
  return (
    <div className={`rounded-md overflow-hidden lg:block ${className}`}>
      <div className="flex justify-between">
        {banners?.exclusiveData.slice(0, 2).map((item: any) => (
          <div
          ...
          >
            ...
            {banners.year && (
              ...
              >
                {item.id !== 2
                  ? banners.year.toString().slice(0, 2)
                  : banners.year.toString().slice(2, 4)}
              </div>
            )}
          </div>
        ))}
